...
## 📦 Сборка пакетов
Сборка всех типов пакетов производится одной командой (требуются rpmbuild и dpkg-deb):
...
* `tftpd-manager_*.deb` — для систем на базе Debian/Ubuntu.

* `tftpd-manager-*-alt.noarch.rpm` — для ALT Linux.

* `tftpd-manager-*-fedora.noarch.rpm` — для Fedora/RedHat.

* `tftpd-manager_*.tar.gz` — универсальный архив.
